...
//  
//
//  Created by Jamin on 3/1/17.
//
//  Makes 1D histograms of peak width.
//
//  I need to convert script so makes 1D hists an delete all colz stuff.
...
int main()
{
  ...
  outFileName = savePath + "D2AZPW99RCentered.root";
  ...
  TH1D *PWidth = new TH1D ("PWidth","PWidth", 500, 600, 1100);
  PWidth->GetXaxis()->SetTitle("drift-time (ns)");
    
  //Loop over files
  UInt_t runNumb;
  for (runNumb = 80; runNumb < 102; runNumb+=2) {
