...
int main()
{
    ...
    if(DSets.find("1") != std::string::npos) {
        chain->Add("/global/projecta/projectdirs/majorana/users/bxyzhu/cal-lat/latSkimDS1*.root");
    ...
    if(DSets.find("5") != std::string::npos) {
        chain->Add("/global/projecta/projectdirs/majorana/users/jrager/Skim/DS5bCal/*.root");
    ...
    outFileName = "DetectorToECalib_" + DSets + "b.root";
    ...
    for(detectorChannelsIt = detectorChannels.begin(); detectorChannelsIt != detectorChannels.end(); detectorChannelsIt++) {
        ...
        ToEHG.insert(std::pair<Double_t, TH2F*>(*detectorChannelsIt,
					      new TH2F(ss.str().c_str(),ss.str().c_str(), 1000, 0., 100, 400, 0., 10)));
    }
	...
	//std::cout << "First for loop." << std::endl;
	  ...
	  if ((*gain)[nhit] == 0 && (*isGood)[nhit] && mH==1 && (*P)[nhit] != 0 && (*D)[nhit] != 0) {
	      ...
	      (ToEHG.find((*channel)[nhit]))->second->Fill((*trapENFCal)[nhit],(*kvorrT)[nhit]/(*trapENFCal)[nhit]);
