...
int main()
{
	...
	detectorChannels.insert(646);
	detectorChannels.insert(644);
    ...
    if(DSets.find("5") != std::string::npos) {
        //for (runNumb = 18623; runNumb <= 22392; runNumb++) {
          //char buffer[50];
	  //int n;
	  ...
	  //std::cout << "Adding run # "  << runNumb << std::endl;
        //}
        chain->Add("/global/projecta/projectdirs/majorana/users/jrager/Skim/DS5aBG/*.root");
	detectorChannels.insert(662);
    ...
    outFileName = "/global/homes/j/jrager/LowE/Data/DetectorTETMvRun_" + DSets + ".root";
	...
	//std::cout << "First for loop." << std::endl;
	  ...
	  if ((*gain)[nhit] == 0 && (*isGood)[nhit] && mH==1 && (*P)[nhit] != 0 && (*D)[nhit] != 0) 
	  {
	      ...
	      (ToEHG.find((*channel)[nhit]))->second->Fill(run,(*RawWFblSlope)[nhit]);
