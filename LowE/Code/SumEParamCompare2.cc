...
int main()
{
    ...
    outFileName = "/global/homes/j/jrager/LowE/ComptonSpecSumEParamThreshDCDS" + DSets + ".root";
    ...
    std::vector<Double_t> *threshKeV = 0;
    chain->SetBranchAddress("threshKeV", &threshKeV);
    std::vector<Double_t> *threshSigma = 0;
    chain->SetBranchAddress("threshSigma", &threshSigma);
    ...
    //EHist->GetYaxis()->SetTitle("sumEH/(trapENFCal[0] + trapENFCal[1])");

    TH1F *EHist = new TH1F("EHist", "EHist", 3000, 0., 3000.);
    EHist->GetXaxis()->SetTitle("trapENFCal (keV)");
    EHist->GetYaxis()->SetTitle("cts");
	...
	if (mH==2 && ((*trapENFCal)[0] > ((*threshKeV)[0] + 3*(*threshSigma)[0])) && ((*trapENFCal)[1] > ((*threshKeV)[1] + 3*(*threshSigma)[1]))){ 
 
	  for (UInt_t nhit = 0; nhit < (*channel).size(); nhit++) {
	    if (sumEH>237.35 && sumEH<239.57 && (*gain)[nhit]==0 && (*isGood)[nhit] && !muVeto && !isLNFill1 && !isLNFill2 && (*P)[nhit]!=0 && (*D)[nhit]!=0){
	      EHist->Fill((*trapENFCal)[nhit]);
	    }
	  }
	}                    
    }
