...
for(int i = 0; i < 20; i++){
	...
	errSumHi[i] = errSumHi[i] + pow(errHi[j][i],2)*pow(...
    ...
    effSum[i] = effSum[i]/4378.6;
    errSumHiSqrt[i] = (sqrt(errSumHi[i]))/4378.6;
    errSumLowSqrt[i] = (sqrt(errSumLow[i]))/4378.6;
    x[i] = i*5;
    cout << "x[i]" << x[i] << endl;
    xerr[i] = 0;
}
