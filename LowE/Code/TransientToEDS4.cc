...
int main()
{
    ...
    if(DSets.find("4") != std::string::npos) {
        //chain->Add("/global/projecta/projectdirs/majorana/users/wisecg/0vbbskim/skimDS4*.root");
	chain->Add("/global/projecta/projectdirs/majorana/users/wisecg/bkg/skim/skimDS4*.root");
    ...
    outFileName = "TransientToEBkg_" + DSets + ".root";
    ...
    for(detectorChannelsIt = detectorChannels.begin(); detectorChannelsIt != detectorChannels.end(); detectorChannelsIt++) {
        ...
        ToEHG.insert(std::pair<Double_t, TH2F*>(*detectorChannelsIt,
						new TH2F(ss.str().c_str(),ss.str().c_str(), 1845, 60000550, 60002395, 400, 0., 10)));
    }
	...
	//std::cout << "First for loop." << std::endl;
	  ...
	  if ((*gain)[nhit] == 0 && (*isGood)[nhit] && mH==1 && (*P)[nhit] != 0 && (*D)[nhit] != 0) {
	      ...
	      (ToEHG.find((*channel)[nhit]))->second->Fill(run,(*kvorrT)[nhit]/(*trapENFCal)[nhit]);
