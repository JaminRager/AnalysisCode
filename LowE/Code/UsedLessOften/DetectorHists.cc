...
int main()
{
	...
	chain->Add("/global/projecta/projectdirs/majorana/users/jrager/Skim/V1Skim/*.root");
        //Enriched detectors
    ...
    outFileName = "DetectorHistsPhysics_" + DSets + ".root";
    ...
    for(detectorChannelsIt = detectorChannels.begin(); detectorChannelsIt != detectorChannels.end(); detectorChannelsIt++) {
        ...
        htrapENFCalHG.insert(std::pair<UInt_t, TH1F*>(*detectorChannelsIt,
                                                      new TH1F(ss.str().c_str(),ss.str().c_str(), 12000, 0., 3000)));
    }
	...
	//std::cout << "First for loop." << std::endl;
	  ...
	  if ((*gain)[nhit]==0 && (*isGood)[nhit] && mH==1 && (*P)[nhit]!=0 && (*D)[nhit]!=0 && !((*channel)[nhit]==1106 && run<26806) && ((*trapETailMin)[nhit]<-0.3 && ((*channel)[nhit]!=660 && (*channel)[nhit]!=1302 && (*kvorrT)[nhit]/((*trapENFCal)[nhit])<2.1 && (*kvorrT)[nhit]/((*trapENFCal)[nhit])>1.1) || (((*channel)[nhit]==660 || (*channel)[nhit]==1302) && (*kvorrT)[nhit]/((*trapENFCal)[nhit])<1.7 && (*kvorrT)[nhit]/((*trapENFCal)[nhit])>0.7))) {
