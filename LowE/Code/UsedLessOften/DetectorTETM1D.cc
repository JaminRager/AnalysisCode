...
int main()
{
    ...
    std::map<Double_t, TH1F*> TETMHG;
    std::map<Double_t, TH1F*>::iterator TETMHGIt;
    ...
    for(detectorChannelsIt = detectorChannels.begin(); detectorChannelsIt != detectorChannels.end(); detectorChannelsIt++) {
        ss.str("");
        ss << "TETMHG" << *detectorChannelsIt;
        TETMHG.insert(std::pair<Double_t, TH1F*>(*detectorChannelsIt,
					      new TH1F(ss.str().c_str(),ss.str().c_str(), 300, -20., 10)));
    }
	...
	//std::cout << "First for loop." << std::endl;
	  ...
	  if ((*gain)[nhit] == 0 && (*isGood)[nhit] && mH==1 && (*P)[nhit] != 0 && (*D)[nhit] != 0)          
	  {
	      ...
	      (TETMHG.find((*channel)[nhit]))->second->Fill((*trapETailMin)[nhit]);
    ...
    for(TETMHGIt = TETMHG.begin(); TETMHGIt != TETMHG.end(); TETMHGIt++){
        TETMHGIt->second->SetOption("colz");
        TETMHGIt->second->Write();
    }
