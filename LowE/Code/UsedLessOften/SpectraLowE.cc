...
int main(){
    ...
    std::string DSets = "6";  // Set to 3 to do DS3, to 4 to do DS4, etc.
    ...
    if(DSets.find("6") != std::string::npos) {
      chain->Add("/global/homes/j/jrager/low_energy_skim/output/*.root");
      //Enriched detectors
      detectorChannels.insert(678); // DS-5 has Module 1 & 2 detectors
      detectorChannels.insert(674);
      detectorChannels.insert(672);
    ...
    for(UInt_t eventNum = 1; eventNum < chain->GetEntries(); eventNum++){
        ...
        for(UInt_t nhit = 0; nhit < (*channel).size(); nhit++) {
            ...
            if(!(*muVeto)[nhit] && (*gain)[nhit] == 0 && (*isGood)[nhit] && !(*wfDCBits)[nhit] && mH==1){
