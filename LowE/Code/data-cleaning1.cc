...
char oldHEtoe[] = "((channel != 664 && channel != 580 && kvorrT/trapENFCal < 1.61 && kvorrT/trapENFCal > 1.13) || (channel == 580 && kvorrT/trapENFCal < 1.2 && kvorrT/trapENFCal > 0.8) || (channel == 664 && kvorrT/trapENFCal < 1.46 && kvorrT/trapENFCal > 1.05))";
char toeKrisCut[] = "kvorrT/trapENFCal>1.1 && kvorrT/trapENFCal<2.1";
char LEtoe[] = "((channel!=580 && channel!=664 && kvorrT/trapENFCal<1.8 && kvorrT/trapENFCal>1.0) || (channel==580 && kvorrT/trapENFCal<1.25 && kvorrT/trapENFCal>0.85) || (channel==664 && kvorrT/trapENFCal<1.55 && kvorrT/trapENFCal>0.9))";
char HEtoe[] = "((channel!=580 && channel!=664 && kvorrT/trapENFCal<1.65 && kvorrT/trapENFCal>1.15) || (channel==580 && kvorrT/trapENFCal<1.15 && kvorrT/trapENFCal>0.85) || (channel==664 && kvorrT/trapENFCal<1.4 && kvorrT/trapENFCal>1.05))"; 
char ToE5k[] = "((channel!=580 && channel!=664 && kvorrT/trapENFCal>0.84 && kvorrT/trapENFCal<1.92) || (channel==580 && kvorrT/trapENFCal>0.65 && kvorrT/trapENFCal<1.37) || (channel==664 &&kvorrT/trapENFCal>0.74 && kvorrT/trapENFCal<1.76))";
...
//char RunDetCut[] = "(!(channel==578 && run==10663) && !(channel==578 && run==11175) && !(channel==692 && run==12746) && !(channel==692 && run>=12765 && run<=12767))";
char cutTwoChan[] = "!(channel==580 || channel==664)";
char twoDet[] = "(channel==580 || channel==664)";
char expRunCutInclude[] = "(run==10663 || run==11175 || run==12746 || (run>=12765 && run<=12767))";
char expRunCutExclude[] = "!(run==10663 || run==11175 || run==12746 || (run>=12765 && run<=12767))";
char ComptonEvents[] = "(mH==2 && sumEH>237.35 && sumEH<239.57 && gain==0 && isGood && !muVeto && !isLNFill1 && !isLNFill2 && P!=0 && D!=0)";
...
void SumEParamRatioVRun()
{
  string outFile = "/global/u1/j/jrager/LowE/SumERatioVRun.root";
  ...
  TH2F *ratio = new TH2F("ratio", "ratio", 5781, 8722, 14503, 3000, 0., 1.);
  ratio->GetXaxis()->SetTitle("run number");
  ratio->GetYaxis()->SetTitle("(trapENFCal[0] + trapENFCal[1])/sumEH");
  skimTree->Project("compare","((trapENFCal[0] + trapENFCal[1])/sumEH):run",cut1);
  ratio->SetOption("colz");
  ratio->Write();
...
void CompareSumEParam()
{
  string outFile = "/global/u1/j/jrager/LowE/CompareSumEParamDS1.root";
...
void BlindRunSelect()
{
  string outFile = "/global/u1/j/jrager/LowE/BlindRunSelectDS1_5kThresh.root";
  ...
  TChain *skimTree = new TChain("skimTree");
  skimTree->Add("/global/projecta/projectdirs/majorana/users/wisecg/bkg/skim/skimDS1*.root");
  //mjdTree->Add("/global/projecta/projectdirs/majorana/data-project/mjd/surfmjd/data/gatified/P3KJR/mjd_run11186.root");
  cout << "Adding data files" <<  endl;
...
void SpectraNoCuts()
{
  string outFile = "/global/u1/j/jrager/LowE/DS1NoCuts.root";
    
  TFile *f = new TFile(outFile.c_str(),"RECREATE");
  cout << "Creating outpout file" <<  endl;
    
  TChain *skimTree = new TChain("skimTree");
  skimTree->Add("/global/projecta/projectdirs/majorana/users/wisecg/bg-skim/skimDS1*.root");
  cout << "Adding data files" <<  endl;
...
void NatToEFinal()
{
  string outFile = "/global/u1/j/jrager/LowE/NatToEDS1Final.root";
...
void EnrToEFinal()
{
  string outFile = "/global/u1/j/jrager/LowE/EnrToEDS1Final.root";
  ...
  //L1->Draw();
  //TLine *L2 = new TLine(0, 1.61, 100, 1.61);
...
void FinalNaturalSpectrum()
{
  string outFile = "/global/u1/j/jrager/LowE/UnscaledNatSpectDS1.root";
  ...
  //sprintf(cut1,"%s && %s && %s && %s && %s",basicCut,tetmOldCut,HEtoe,HEDetCut,natCut);
  //sprintf(cut2,"%s && %s && %s && %s && %s",basicCut,tetmOldCut,toeCut,HEDetCut,natCut);
  //Double_t DS1exposureEnr = 4/679.307;
...
void ToEAllCuts()
{
  string outFile = "/global/u1/j/jrager/LowE/FinalToEDS1.root";

  TFile *f = new TFile(outFile.c_str(),"RECREATE");
  cout << "Creating outpout file" <<  endl;

  TChain *skimTree = new TChain("skimTree");
  skimTree->Add("/global/homes/w/wisecg/project/bg-skim/skimDS1_*_low.root");
  cout << "Adding data files" <<  endl;
...
void DetectorSelection()
{
  string outFile = "/global/u1/j/jrager/LowE/DetSelectDS1_5kThresh.root";
  ...
  TH2F *detVrun = new TH2F("detVrun", "detVrun", 5781, 8722, 14503, 800, 550, 1350);
...
void RunSelection()
{
  string outFile = "/global/u1/j/jrager/LowE/RunSelectDS1_5kThresh.root";
  ...
  TH2F *EVrun = new TH2F("EVrun","EVrun", 5781, 8722, 14503, 2000, 0, 500);
  ...
  TH2F *EVrunBasic = new TH2F("EVrunBasic","EVrunBasic", 5781, 8722, 14503, 2000, 0, 500);
  ...
  TH2F *EVrunAdv = new TH2F("EVrunAdv","EVrunAdv", 5781, 8722, 14503, 2000, 0, 500);
  ...
  TH2F *EVrunOld = new TH2F("EVrunOld","EVrunOld", 5781, 8722, 14503, 2000, 0, 500);
...
void TestWFCuts()
{
    string outFile = "/global/u1/j/jrager/LowE/TestWFCutsDS1.root";
...
void TuneToECut1()
{
  string outFile = "/global/u1/j/jrager/LowE/TuneToECutDS1_cutTwoChan.root";
...
void TuneToECut2()
{
  string outFile = "/global/u1/j/jrager/LowE/TuneToECutDS1_chan580.root";
...
void TuneToECut3()
{
  string outFile = "/global/u1/j/jrager/LowE/TuneToECutDS1_chan664_Compton.root";
...
void TuneWFCuts2()
{
  string outFile = "/global/u1/j/jrager/LowE/TuneWFCutsDS1_cut2Chan_Compton.root";
...
void TETMCutStudy2D()
{
    ...
    string outFile = "/global/u1/j/jrager/LowE/TETMstudyImprovedDS1.root";
    ...
    TH2F *EVrun = new TH2F("EVrun","EVrun", 5781, 8722, 14503, 120, 0, 30);
    EVrun->GetYaxis()->SetTitle("trapENFCal (keV)");
    EVrun->GetXaxis()->SetTitle("run");
...
void TETMCutStudy1D()
{
    string outFile = "/global/u1/j/jrager/LowE/TETMcutRecImprovedDS1.root";
    ...
    TH1F *htrapETailMin = new TH1F("htrapETailMin","htrapETailMin", 200, -10, 10);
    htrapETailMin->GetXaxis()->SetTitle("trapETailMin");
    //sprintf(theCut,"%s && %s", basicCut, wfdcCut);
    skimTree->Project("htrapETailMin","trapETailMin",theCut);
    htrapETailMin->Write();
...
void RunsAndDets()
{
  string outFile = "/global/u1/j/jrager/LowE/RunSellectDS1.root";
  ...
  TH1F *hrun = new TH1F("hrun", "hrun", 5781, 8722, 14503);
