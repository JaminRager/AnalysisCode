...
int main(int argc, char **argv) {
  ...
  MJRunInfo  runInfo;
  int        nDets, i, argn=1, nEnab=0, file, file_is_a_pipe=0;
  ...
  } else {                                    // use command argument list for input files
  ...
  for (i=0; i<nDets; i++) {
    if (i%20 == 0)
      printf("#    DetID      pos      name     HiGain Enab Thresh     HVch  MaxV Target"
             "     Pulser times enab  ampl atten  CC\n");
    printf(" %3d  was %2d %8s %9s   %d,%2.2d,%d %4d %6d %3d,%2.2d,%d"
  ...
  if (runInfo.dataIdG == 0) {
    printf("\n No data ID found for Gretina4M data!\n");
    return 1;
  }
#ifndef QUIET
  else {
  ...
  while (1) {
    ...
    strncpy(runInfo.filename, fname, sizeof(runInfo.filename));
    printf("\n >>> Reading %s\n\n", fname);
    fread(&runInfo.fileHeaderLen, 1, 1, f_in);
