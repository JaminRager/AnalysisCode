...
//
//  Makes 2D histograms of drift time vs. z with no NaI cuts.
...
int main()
{
  ...
  outFileName = savePath + "noNaICuts.root";
  ...
  for (runNumb = 35006441; runNumb < 35006460; runNumb++, z+=2) {
    ...
    Double_t dt = 0;
    Double_t x = 38-z-2; //I plot against this variable instead of Z to invert histogram.
    ...
    for(UInt_t eventNum = 1; eventNum < chain->GetEntries(); eventNum++){
      ...
      if((*channel)[0]==160 && (*energy)[0]>20900 && (*energy)[0]<21500){
	dt = (*blrwfFSR50)[0] - (*negblrmFSR10)[1];
	DriftT->Fill(x, dt);
      }
        
      else if((*channel)[0]==166 && (*energy)[1]>20900 && (*energy)[1]<21500){
