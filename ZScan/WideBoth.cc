...
//  
//
//  Created by Jamin on 2/23/17.
//
//  Makes 2D histograms of drift time vs. z with wide NaI cuts.
...
int main()
{
  ...
  outFileName = savePath + "WideBoth.root";
  ...
  TH2D *DriftT = new TH2D("DriftT", "DriftT", 19, 0, 38, 25, 0., 1200. );
  ...
  UInt_t runNumb;
  UInt_t z = 0;
  for (runNumb = 35006441; runNumb < 35006460; runNumb++, z+=2) {
    ...
    std::vector<Double_t> *negblrmFSR10 = 0;
    chain->SetBranchAddress("negblrmFSR10", &negblrmFSR10);
    std::cout << "Finished Assigning Variables for TTree output." << std::endl;
        
    // Create drift time variable "dt"
    Double_t dt = 0;
    Double_t x = 38-z-2; //Inverts histogram.
    ...
    for(UInt_t eventNum = 1; eventNum < chain->GetEntries(); eventNum++){
      ...
      if((*channel)[0]==160 && (*energy)[0]>20000 && (*energy)[0]<23000 && (*energy)[1]>=3000 && (*energy)[1]<=8000){
	dt = (*blrwfFSR50)[0] - (*negblrmFSR10)[1];
	DriftT->Fill(x, dt);
      }
        
      else if((*channel)[0]==166 && (*energy)[1]>20000 && (*energy)[1]<23000 && (*energy)[0]>=3000 && (*energy)[0]<=8000){
	dt = (*blrwfFSR50)[1] - (*negblrmFSR10)[0];
	DriftT->Fill(x, dt);
      }
