...
int main(int argc, const char** argv)
{
  if (argc < 3 || argc > 8) {
    cout << "Usage:  ./skim_mjd_data [options] [output path (optional)]\n"
	 ...
	 << "   [-d] (Use double type for integer branches such as run number and channel. This is needed if gatified file used GAT tag 1.06 or older) \n";
    return 1;
  }
  // ==========================================================================
  // Get user arguments
  ...
  bool smallOutput=0, simulatedInput=0, singleFile=0, lowEnergy=0, noSkip=1, useDoubles=0;
  ...
  while(gatReader.Next())
  {
	...
	channelList.push_back(int(chan));
    } else {
    ...
    if(*eventDC1BitsIn & kPinghanPulserMask)
    {
      ...
      // cout << (int)dummyDTGlobal << endl;
  		for (size_t i = 0; i < channelList.size(); i++) {
  			if(pulserCardMap[channelList[i]] == 0) continue; // Skip PMon channels
  			dummydTCard[ pulserCardMap[channelList[i]] ] = (double)*globalTimeIn + (*tOffsetIn)[i]/CLHEP::s;
  		}
      continue;
    }
    ...
    if (!noSkip)
    {
      ...
      for (size_t i = 0; i < channelList.size(); i++) {
        if (find(pulserMons.begin(),pulserMons.end(),channelList[i])!=pulserMons.end()) continue;
        chIdxMap[channelList[i]] = (int)i;
      }

      // Loop over the raw list of channels
      for (size_t i=0; i < channelList.size(); i++)
      {
        int chan = channelList[i];
